...
const ReturnsAndOrders = () => {
  ...
  useEffect(() => {
    ...
    const viewedProductsArray = viewedProductsCookie
      ? JSON.parse(viewedProductsCookie)
      : [];
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4 text-center">
        Viewed items in your{" "}
        <span className="text-[#FACC15]">browsing history</span> also viewed
      </h1>
        ...
        {currentProducts.length > 0 ? (
          currentProducts.map((product) => (
            <Link
              ...
              key={product._id}
              className="border p-4 rounded-lg bg-white shadow-xl 
                         w-full sm:w-[48%] md:w-[30%] lg:w-[22%] xl:w-[18%]" // Responsive widths
            >
              <img
          ...
          <div className="flex flex-col items-center justify-center h-48 mx-auto text-center w-full sm:w-3/4">
            <BiCart className="text-6xl text-gray-400" />{" "}
            {/* Large cart icon */}
      ...
      <div className="flex justify-center items-center mt-4">
        ...
        <button
          onClick={handleNext}
          disabled={
            (currentPage + 1) * productsPerPage >= viewedProducts.length
          }
