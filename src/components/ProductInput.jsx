...
const ProductInput = () => {
  ...
  useEffect(() => {
    if (selectedCategory) {
      const fetchSubCategories = async () => {
        try {
          const response = await axios.get(`http://localhost:5000/api/categories/${selectedCategory}`);
  ...
  useEffect(() => {
    if (formData.subCategory) {
      const selectedSubCategory = subCategories.find(sc => sc._id === formData.subCategory);
      setSubSubCategories(selectedSubCategory ? selectedSubCategory.subSubCategories : []);
    } else {
      setSubSubCategories([]);
    }
