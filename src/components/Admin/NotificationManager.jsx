...
const NotificationManager = () => {
  ...
  const [editedNotification, setEditedNotification] = useState({ message: '', type: '' });
  ...
  const handleEditChange = (e) => {
    setEditedNotification({ ...editedNotification, [e.target.name]: e.target.value });
  };
                ...
                {notifications.map((notification, index) => (
                  <tr key={notification._id} className="even:bg-gray-50">
                    ...
                    <td className="border border-gray-300 px-4 py-2">
                      <button
                        onClick={() => { setEditing(notification); setEditedNotification(notification); }}
